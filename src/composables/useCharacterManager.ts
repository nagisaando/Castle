...
export function useCharacterManager() {
  const { createShadow } = useShadowManager();
  ...
  const initMouse = (
    ...
    const sphereRadius =
      Math.max(bodySize.x, bodySize.y, bodySize.z) * 0.5 * 0.8; // 80% of half the largest dimension

    // Create shadow using shadow manager
    const shadow = createShadow(mouseModel, mouseBody, scene);
    ...
    const mouseBoundingSphere = new THREE.Sphere(
      bodyWorldPosition,
      sphereRadius
    );
